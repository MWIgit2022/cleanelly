...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	*/
	...
	/**
	 * собираются комбинации всех свойств ТП товара, все комбинации добавляются в массив $arResult, 
	 * в массиве $propertyValues находятся свойства всех ТП, не распределенные по комбинациям для фильтра
	 *
	 * @param  array $arResult
	 * @param  array $propertyValues 
	 */
	function makeCombinations(&$arResult, $propertyValues) {

		$combinations = [];//В этот массив собираются комбинации для всех ТП одного товара
		$defaultCombination = [];//Массив значений для первого элемента
		if (!empty($propertyValues)) {

			foreach ($propertyValues as $key=>$valuesSetForOneProperty) {//цикл по свойствам, $valuesSetForOneProperty - набор значений одного свойства (значений может быть больше одного)
				if (is_array($valuesSetForOneProperty) && !empty($valuesSetForOneProperty) && !empty(current($valuesSetForOneProperty))) {
					$defaultCombination[$key] = array_shift($valuesSetForOneProperty);//создается первая комбинация по умолчанию для первого (или единственного) ТП товара

					$i = 0;
					foreach ($valuesSetForOneProperty as $value) {//цикл по значениям свойств для остальных ТП товара
			...
			$arResult[] = $defaultCombination;//первая комбинация добавляются в $arResult

			if (!empty($combinations)) {
				foreach ($combinations as $key=>$combination) {//цикл по комбинациям
					foreach ($defaultCombination as $valueKey=>$value) {//цикл по свойствам из первой комбинации
						if (empty($combination[$valueKey])) {			//если свойства нет у остальных комбинаций (определяется это по id свойств - $valueKey)
							$combinations[$key][$valueKey] = $value;    //то во все остальные комбинации добавляются общие для всех ТП свойства
						}
					}

					$arResult[] = $combinations[$key];//все комбинации для всех ТП одного товара добавляются в $arResult
				}
			}
		}
	}
