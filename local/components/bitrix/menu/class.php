	...
	public function getChildMenuRecursive(&$arMenu, &$arResult, $menuType, $use_ext, $menuTemplate, $currentLevel, $maxLevel, $bMultiSelect, $bCheckSelected, $parentItem)
	{
		...
		for ($menuIndex = 0, $menuCount = count($arMenu); $menuIndex < $menuCount; $menuIndex++)
		{
			...
			if (is_array($arMenu[$menuIndex]["PARAMS"]) && isset($arMenu[$menuIndex]["PARAMS"]["FROM_IBLOCK"]))
			{
				...
				$menu_item_name = $arMenu[$menuIndex]["MENU_ITEM_NAME"] ? $arMenu[$menuIndex]["MENU_ITEM_NAME"] : $arMenu[$menuIndex]["PARAMS"]["MENU_ITEM_NAME"];
				$arResult[] = $arMenu[$menuIndex] + Array("DEPTH_LEVEL" => $iblockSectionLevel, "IS_PARENT" => $arMenu[$menuIndex]["PARAMS"]["IS_PARENT"], 'MENU_ITEM_NAME' => $menu_item_name);
				continue;
			}
			...
			if(!$subMenuExists)
				$arResult[] = $arMenu[$menuIndex] + Array("DEPTH_LEVEL" => $currentLevel, "IS_PARENT" => false, 'MENU_ITEM_NAME' => $arMenu[$menuIndex]["PARAMS"]["MENU_ITEM_NAME"]);
		}
	}
