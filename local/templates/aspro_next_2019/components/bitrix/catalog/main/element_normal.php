...
$arParams['PRODUCT_QUANTITY_VARIABLE'] = 'N';
...
$sViewElementTemplate = ($arParams["ELEMENT_TYPE_VIEW"] == "FROM_MODULE"
	? $arTheme["CATALOG_PAGE_DETAIL"]["VALUE"]
	: $arParams["ELEMENT_TYPE_VIEW"]);

$hide_left_block = ($arTheme["LEFT_BLOCK_CATALOG_DETAIL"]["VALUE"] == "Y" ? "N" : "Y");
$arWidePage = ["element_3", "element_4", "element_5"];
...
if ($arSection['UF_ELEMENT_DETAIL']) {
	...
	if ($arSection["DEPTH_LEVEL"] > 2) {
		$sectionParent = CNextCache::CIBlockSection_GetList([
			'CACHE' => [
				"MULTI" => "N",
				"TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])
			]
		], ['GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]],
			false, ["ID", "IBLOCK_ID", "NAME", "UF_ELEMENT_DETAIL"]);
		...
		if (!$typeTmpDetail) {
			$sectionRoot = CNextCache::CIBlockSection_GetList([
				'CACHE' => [
					"MULTI" => "N",
					"TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])
				]
...
if ($typeTmpDetail) {
	$rsTypes = CUserFieldEnum::GetList([], ["ID" => $typeTmpDetail]);
...
<? if ($arParams["USE_SHARE"] == "Y" && $arElement && !in_array($sViewElementTemplate, $arWidePage)): ?>
	...
	<div class="line_block share top <?= ($arParams['USE_RSS'] == 'Y' ? 'rss-block' : ''); ?>">
		<? $APPLICATION->IncludeFile(SITE_DIR . "include/share_buttons.php", [],
			["MODE" => "html", "NAME" => GetMessage('CT_BCE_CATALOG_SOC_BUTTON')]); ?>
	</div>
	<? $this->EndViewTarget(); ?>
<? endif; ?>
...
<? endif; ?>
<div class="catalog_detail detail<?= ($isWideBlock == "Y" ? " fixed_wrapper" : ""); ?> <?= $sViewElementTemplate; ?>"
...
<?
$APPLICATION->ShowViewContent('element_describe');

$arAllValues = $arSimilar = $arAccessories = [];
$arShowTabs = ["element_1", "element_2"];
if (!in_array($sViewElementTemplate, $arWidePage)):
	/*similar goods*/
	$arExpValues = CNextCache::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID,
		["CACHE" => ["TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ["CODE" => "EXPANDABLES"]);
	if ($arExpValues) {
		$arAllValues["EXPANDABLES"] = $arExpValues;
	}
	/*accessories goods*/
	$arAccessories = CNextCache::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID,
		["CACHE" => ["TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ["CODE" => "ASSOCIATED"]);
	if ($arAccessories) {
							...
							<li class="tab <?= $code ?>_wrapp cur" data-code="<?= $code ?>">
								...
								<?
									...
									<div class="flexslider loading_state shadow border custom_flex top_right"
										 data-plugin-options='{"animation": "slide", "animationSpeed": 600, "directionNav": true, "controlNav" :false, "animationLoop": true, "slideshow": false, "controlsContainer": ".tabs_slider_navigation.<?= $code ?>_nav", "counts": [4,3,3,2,1]}'>
										<ul class="tabs_slider <?= $code ?>_slides slides">
											<? $GLOBALS['arrFilter' . $code] = ["XML_ID" => $arAllValues[$code]]; ?>
...
if ($arNavParams) {
	$arAdditionalData['NAV_CHAIN'] = $APPLICATION->GetNavChain($arNavParams[0], $arNavParams[1], $arNavParams[2],
		$arNavParams[3], $arNavParams[4]);
}
?>
<script type="text/javascript">
