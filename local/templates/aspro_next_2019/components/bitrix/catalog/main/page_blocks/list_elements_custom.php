				...
				<?if($arParams["SHOW_SECTION_DESC"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false):?>
					...
					<?elseif($posSectionDescr=="TOP"):?>
						<?if ($arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]):?>
							...
							<span id="open_the_text">Развернуть</span>
			...
			<?}?>
				<?$APPLICATION->IncludeComponent(
					...
					Array(
						...
						"ELEMENT_SORT_FIELD" => $sort === 'ID' ? $arParams["ELEMENT_SORT_FIELD2"] : $sort,
						"ELEMENT_SORT_ORDER" => $sort === 'ID' ? $arParams["ELEMENT_SORT_ORDER2"] : $sort_order,
