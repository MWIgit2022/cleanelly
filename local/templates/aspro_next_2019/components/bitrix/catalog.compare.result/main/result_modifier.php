...
foreach ($arResult["ITEMS"] as &$arItem) {
	...
	$arSKU = CCatalogSKU::getOffersList($arItem["ID"], 0, [], [], []);
	foreach ($arSKU[$arItem["ID"]] as $ofkey => $arOfferItem) {
		$OfferID = $ofkey;
	}
	$res = CIBlockElement::GetProperty($arResult["OFFERS_IBLOCK_ID"], $OfferID, [], $arParams["OFFERS_PROPERTY_CODE"]);
	while ($ob = $res->Fetch())
	{
		if (array_search($ob["CODE"], $arParams["OFFERS_PROPERTY_CODE"])) {
			if ($ob["VALUE_ENUM"]) {
				$arItem["OFFER_DISPLAY_PROPERTIES"][$ob["CODE"]]["NAME"] = $ob["NAME"];
				$arItem["OFFER_DISPLAY_PROPERTIES"][$ob["CODE"]]["DISPLAY_VALUE"] = $ob["VALUE_ENUM"];
			} elseif ($ob["VALUE"]) {
				$arItem["OFFER_DISPLAY_PROPERTIES"][$ob["CODE"]]["NAME"] = $ob["NAME"];
				$arItem["OFFER_DISPLAY_PROPERTIES"][$ob["CODE"]]["DISPLAY_VALUE"] = $ob["VALUE"];
			}
		}
	}
