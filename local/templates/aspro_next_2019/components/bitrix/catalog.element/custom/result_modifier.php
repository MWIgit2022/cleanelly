...
if($productSlider){
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				"BIG" => CFile::ResizeImageGet($arImage["ID"], array("width" => 870, "height" => 870), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 870, "height" => 870), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
				"THUMB" => CFile::ResizeImageGet($arImage["ID"], array("width" => 105, "height" => 105), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
			)
		);
	}
}
...
//$arParams['USE_ADDITIONAL_GALLERY'] = 'N';
/* ADDITIONAL GALLERY */
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as &$arOffer)
		{
			if($arOffer['PROPERTIES']['NE_OTBELIVAT']['VALUE'] == 'Да'){
				$arResult['UHOD']['NE_OTBELIVAT'] = $arOffer['PROPERTIES']['NE_OTBELIVAT'];
			}
			if($arOffer['PROPERTIES']['KHIMCHISTKA_ZAPRESHCHENA']['VALUE'] == 'Да'){
				$arResult['UHOD']['KHIMCHISTKA_ZAPRESHCHENA'] = $arOffer['PROPERTIES']['KHIMCHISTKA_ZAPRESHCHENA'];
			}
			if($arOffer['PROPERTIES']['STIRKA_40S']['VALUE'] == 'Да'){
				$arResult['UHOD']['STIRKA_40S'] = $arOffer['PROPERTIES']['STIRKA_40S'];
			}
			if($arOffer['PROPERTIES']['NE_GLADIT']['VALUE'] == 'Да'){
				$arResult['UHOD']['NE_GLADIT'] = $arOffer['PROPERTIES']['NE_GLADIT'];
			}
			
			if($arOffer['PROPERTIES']['STIRKA_60S']['VALUE'] == 'Да'){
				$arResult['UHOD']['STIRKA_60S'] = $arOffer['PROPERTIES']['STIRKA_60S'];
			}
			if($arOffer['PROPERTIES']['GLAZHKA_150S']['VALUE'] == 'Да'){
				$arResult['UHOD']['GLAZHKA_150S'] = $arOffer['PROPERTIES']['GLAZHKA_150S'];
			}
			if($arOffer['PROPERTIES']['VOZMOZHNA_BARABANNAYA_SUSHKA']['VALUE'] == 'Да'){
				$arResult['UHOD']['VOZMOZHNA_BARABANNAYA_SUSHKA'] = $arOffer['PROPERTIES']['VOZMOZHNA_BARABANNAYA_SUSHKA'];
			}
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					$arOffer['MORE_PHOTO'][$i]["BIG"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 870, "height" => 870), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			{
				$article = $arOffer['PROPERTIES']["BARCODE"]["VALUE"];
			}
...
$arResult["HAS_FILL_PROPERTIES"] = false;
if (!empty($arResult["PROPERTIES"]))
{
	foreach ($arResult["PROPERTIES"] as $code => $val)
	{
		if (!empty($val["VALUE"]))
		{
			$arResult["HAS_FILL_PROPERTIES"] = true;
			break;
		}
	}
}
...
if (in_array($arResult['ID'], $favorites->getFavoritesList())) {
	$arResult['FAVORITE'] = 'Y';
} else {
	$arResult['FAVORITE'] = 'N';
}
...
// видео 
foreach ($arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'] as $key => $videoCode) {
	$videoCode = preg_split('/;/', $videoCode);
	$arResult['VIDEO_URLS'][$key] = 'https://www.youtube.com/embed/' . $videoCode[0]  . '?autoplay=0';

	$urlForPreview = 'https://www.youtube.com/watch?v=' . $videoCode[0];
	$youtubeLink = "https://www.youtube.com/oembed?url=". $urlForPreview ."&format=json";
