...
<div class="form <?=$arResult["arForm"]["SID"]?>">
	...
	<div class="form_head">
        <?if($arResult["isFormTitle"] == "Y"):?>
            <h2 class="<?=($arResult["WEB_FORM_NAME"] == "NEW_CLIENT")?'title-form__new-client':''?>"><?=$arResult["FORM_TITLE"]?></h2>
        <?endif;?>
        <?if($arResult["isFormDescription"] == "Y" && $arResult["WEB_FORM_NAME"] != "NEW_CLIENT"):?>
            <div class="form_desc"><?=$arResult["FORM_DESCRIPTION"]?></div>
        <?elseif($arResult["WEB_FORM_NAME"] == "NEW_CLIENT"):?>
			<p class="new_client_form_top_notice">Скидка 10% по промокоду действует на товары, которые на момент его использования не имеют скидки (только на нескидочные товары).</p>
        <?endif;?>
