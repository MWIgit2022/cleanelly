		...
		<table>
			<tr>
				<?foreach($arResult as $arItem):?>					
					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?> <?=($arItem["LINK"] == "/catalog/zelenyy_den_sberbank/" ? "sberbank" : "")?> <?=($arItem["LINK"] == "/catalog/cleanelly_green_planet/" ? "sberbank" : "")?>">
						<div class="wrap">
							<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" href="<?=$arItem["LINK"]?>">
								<div class="if_hover_img_event" <?if($arItem['PARAMS']["MENU_ICON"]){?> style="padding-right:30px;"<?}?>>
								<?if($arItem['PARAMS']["MENU_ICON"]){?>
									<img src="<?=Cfile::getPath($arItem['PARAMS']["MENU_ICON"])?>" style="max-width:20px;margin-right:0.5em;">
									<img src="<?=Cfile::getPath($arItem['PARAMS']["HOVER_MENU_ICON"])?>" style="max-width:20px;margin-right:0.5em;display:none;">
								<?}?>
									<?=$arItem["TEXT"]?>
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<ul class="dropdown-menu <?if($arItem['TEXT'] == 'Полотенца'){?>polotentsa_menu<?}?>">
									<?foreach($arItem["CHILD"] as $arSubItem):?>
										...
										<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=(($arSubItem["SELECTED"] && !$bShowChilds)? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?>">
											...
											<? if($arSubItem["CHILD"] && $bShowChilds) { ?>
												<span class="name group-name"><?=$arSubItem["TEXT"]?></span>
											<? } else { ?>
											...
											<?if($arSubItem["CHILD"] && $bShowChilds):?>
												...
												<ul class="toggle_menu">
													<?foreach($arSubItem["CHILD"] as $key => $arSubSubItem):?>
														...
														<li class="menu-item <?=(++$key > $iVisibleItemsMenu ? 'collapsed' : '');?> <?=($arSubSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=(($arSubSubItem["SELECTED"] && ! $bShowChilds) ? "active" : "")?>">
