...
if(!funcDefined('initSelects')){
	function initSelects(target){
		$(target).find('.wrapper1 select:visible').each(function(){
			if($(this).hasClass('city') || $(this).hasClass('region')) {
				$(this).select2();
			}
		});
	}
				...
				if($('#one_click_buy_form').valid())
				{
					if($('.'+name+'_frame form input.error').length || $('.'+name+'_frame form textarea.error').length) {
						...
						console.log('send-metrics'); 
                                                ...
                                                console.log("ym(22769200, 'reachGoal', 'fast_order')");
						ym(22769200, 'reachGoal', 'fast_order');
                                                
						console.log("gtag('event', 'fast_order')");
						gtag('event', 'fast_order');
                                                
						console.log('end-send-metrics');
...
if(!funcDefined("addBasketCounter")){
	function addBasketCounter(id){
		...
		if(checkCounters()){
			$(".show_popup_add_to_cart").trigger("click");
			$.ajax({
	...
	$('select.city, select.region').on('change', function(){
		...
		if(_this.hasClass('city')) {
			if(_this.find('option:selected').length > 0) {
				type = _this.find('option:selected').data('type').toLowerCase();
			}
		}
		
		if((_this.hasClass('region') || _this.hasClass('city')) && val)
		{
			$.ajax({
				type: 'POST',
				...
				if(type == 'region') {
					...
					$('select.region').prop('selectedIndex', 0);
...
$(document).ready( initAllElements );
...
if(!funcDefined('orderActions')){
		...
		if(arNextOptions["PAGES"]["ORDER_PAGE"]){
			...
			if($('.bx-soa-cart-total').length){
				if(!$('.change_basket').length) {
					...
					$($totalLines).each(function(index, element) {
						$(element).append(promocodeDiv);
					});

					initializePromocodeInput();
	...
	promocodeFieldUpdate = function()
	{
		let customerFieldPromocode = $('body').find('input[name="promocode"]');
		$('body').on('click', '.order-promocode-block-promocode-btn', function() {
			customerFieldPromocode.each(function() {
				if ($(this).val()) {
					let valueOfField = $(this).val();
					if (!/[0-9]{13}/.test(valueOfField)) {
						let urlAjax = "/ajax/applyPromocodeOnOrderPage.php";
	...
	initializePromocodeInput = function() {
		let coupons = BX.Sale.OrderAjaxComponent.result.COUPON_LIST;
		let customerFieldPromocode = $('body').find('input[name="promocode"]');
		$.each(coupons, function() { 
			var currentCopon = $(this)[0];
			if (currentCopon.JS_STATUS == "APPLIED" && !/[0-9]{13}/.test(currentCopon.COUPON)) {
				customerFieldPromocode.val(currentCopon.COUPON);
			}
		});
	}

	addPromocodeMessages = function()
	{
		let coupons = BX.Sale.OrderAjaxComponent.result.COUPON_LIST;
		var promocodeAlertInner = $('body').find('.order-promocode-alert-section');
		$.each(coupons, function() {
			var currentCopon = $(this)[0];
			if (currentCopon.JS_STATUS == "APPLIED") {
				promocodeAlertInner.append('<div class="order-promocode-alert order-text-muted" data-promocodediv="'+currentCopon.COUPON+'"></div>');																
				promocodeAlertInner.find('.order-text-muted.order-text-muted').append('<span class="order-promocode-text"><strong>'+currentCopon.COUPON+'</strong> - промокод применен '+currentCopon.DISCOUNT_NAME+'</span>');
				promocodeAlertInner.find('.order-text-muted.order-text-muted').append('<span class="close-link-promocode" data-entity="order-promocode-delete" data-promocode="'+currentCopon.COUPON+'">Удалить</span>');
			} else if (currentCopon.ACTIVE == "N") {
				promocodeAlertInner.append('<div class="order-promocode-alert order-text-danger"><span class="order-promocode-text"><strong>'+currentCopon.COUPON+'</strong> - купон не активен '+currentCopon.DISCOUNT_NAME+'</span><span class="close-link-promocode" data-entity="order-promocode-delete" data-promocode="'+currentCopon.COUPON+'">Удалить</span> </div>');
			} else if (currentCopon.JS_STATUS == "BAD") {
				promocodeAlertInner.append('<div class="order-promocode-alert order-text-danger"><span class="order-promocode-text"><strong>'+currentCopon.COUPON+'</strong> - ' + currentCopon.JS_CHECK_CODE + ' (' +currentCopon.DISCOUNT_NAME+') </span><span class="close-link-promocode" data-entity="order-promocode-delete" data-promocode="'+currentCopon.COUPON+'">Удалить</span> </div>'); 
			} else if (currentCopon.DISCOUNT_ACTIVE == "Y") {
				promocodeAlertInner.append('<div class="order-promocode-alert order-text-danger"><span class="order-promocode-text"><strong>'+currentCopon.COUPON+'</strong> - промокод не применён, т.к. не выполнены все условия, либо активирован другой. По вопросам применения промокода можно обратиться на горячую линию интернет-магазина '+currentCopon.DISCOUNT_NAME+'</span><span class="close-link-promocode" data-entity="order-promocode-delete" data-promocode="'+currentCopon.COUPON+'">Удалить</span> </div>');
			}
		});
	}

	deletePromocode = function() {
		$('body').on('click', '.close-link-promocode', function(){
			let urlAjax = "/ajax/deletePromocodeOnOrderPage.php";
			let valueOfField = $(this).data('promocode');
			BX.ajax.loadJSON(urlAjax,{
				AJAX:'Y',
				data: valueOfField,
				action: 'deleteCoupon'
...
$(document).ready(function() {
	$(document).on("click", ".show_popup_add_to_cart", function() {
		if ($(".popup_add_to_cart").is(":visible")){
	...
	$(document).on("click", ".close", function() {
		if ($(this).parent().hasClass("popup_add_to_cart")){
			$(".show_popup_add_to_cart").trigger("click");
		}
	});
	$(document).on("click", ".btn_to_close", function() {
		if ($(this).parents().hasClass("popup_add_to_cart")){
			$(".show_popup_add_to_cart").trigger("click");
		}
	});
	$(document).on("change", ".bx-soa-pp-company-select", function(){
		$("input[name=ORDER_PROP_29]").val($(this).val());
	});
