var basketTimeoutSlide;var resizeEventTimer;var funcDefined=function(func){try
{if(typeof func=='function')
return!0;else return typeof window[func]==="function"}
catch(e)
{return!1}}
...
else{$('.opener .basket_count').removeClass('small')
...
else{$('.opener .wish_count').removeClass('small')
...
else{if(arNextOptions.THEME.SHOW_BASKET_ONADDTOCART!=='N')
$('.opener .basket_count').click()}}}}))}}
...
else{if($('#basket_toolbar_button').length)
...
else{$("#basket_line .basket_fly .tabs li").first().addClass("cur").siblings().removeClass("cur");$("#basket_line .basket_fly .tabs_content li").first().addClass("cur").siblings().removeClass("cur");$("#basket_line .basket_fly .remove_all_basket").first().addClass("cur")}
...
else{$.ajax({url:arNextOptions.SITE_DIR+'ajax/basket_fly.php',type:'post',success:function(html){$('#basket_line .basket_fly').addClass('loaded').html(html);onOpenFlyBasket(_this)}})}})}
...
delete arViewedLocal[2243];delete arViewedCookie[2243];var time=new Date().getTime();arData.ID=id;arData.ACTIVE_FROM=time;arData.IS_LAST=!0;arViewedLocal[PRODUCT_ID]=arData;arViewedCookie[PRODUCT_ID]=[time.toString(),arData.PICTURE_ID];$.cookie(localKey,arViewedCookie,cookieParams);BX.localStorage.set(localKey,arViewedLocal,2592000)}}
catch(e){console.error(e)}
...
calc_height=$(e).actual('outerHeight');else calc_height=$(e).actual('height');return calc_height}).get();for(var i=0,c=maxHeight.length;i<c;++i)
{if(maxHeight[i]%2)
--maxHeight[i]}
return Math.max.apply(this,maxHeight)}
$.fn.equalizeHeights=function(outer,classNull,minHeight){var maxHeight=this.map(function(i,e){var minus_height=0,calc_height=0;if(classNull!==!1)
{if(!isMobile)
minus_height=parseInt($(e).find(classNull).actual('outerHeight'))}
if(minus_height)
minus_height+=12;$(e).css('height','');if(outer==!0)
calc_height=$(e).actual('outerHeight')-minus_height;else calc_height=$(e).actual('height')-minus_height;if(minHeight!==!1)
...
return width}
$.fn.sliceHeight=function(options){function _slice(el){el.each(function(){$(this).css('line-height','');$(this).css('height','')});if(options.mobile==!0)
...
{var arBreakpoints=Object.keys(options.breakpoint),bSliceNext=!1;if(arBreakpoints.length)
...
{bSliceNext=!0;options.slice=options.breakpoint[arBreakpoints[key]]}}}}
if(typeof(options.typeResize)=='undefined'||options.typeResize==!1)
...
var options=$.extend({slice:null,sliceNext:null,outer:!1,lineheight:!1,autoslicecount:!0,classNull:!1,minHeight:!1,row:!1,item:!1,typeResize:!1,typeValue:!1,fixWidth:0,resize:!0,mobile:!1,customSlice:!1,breakpoint:{},},options);var el=$(this);ignoreResize.push(!0);_slice(el);ignoreResize.pop();if(options.resize)
{BX.addCustomEvent('onWindowResize',function(eventdata){try{ignoreResize.push(!0);_slice(el)}
catch(e){}
finally{ignoreResize.pop()}})}
else{if(!ignoreResize.length)
{_slice(el)}}}
$.fn.sliceHeightNoResize=function(options){function _slice(el){el.each(function(){$(this).css('line-height','');$(this).css('height','')});if(typeof(options.autoslicecount)=='undefined'||options.autoslicecount!==!1){var elw=(el.first().hasClass('item')?el.first().getFloatWidth():el.first().parents('.item').getFloatWidth());var elsw=el.first().parents('.items').getFloatWidth();if(!elsw){elsw=el.first().parents('.row').getFloatWidth()}
...
if(options.lineheight){var lineheightAdd=parseInt(options.lineheight);if(isNaN(lineheightAdd)){lineheightAdd=0}
el.each(function(){$(this).css('line-height',($(this).actual('height')+lineheightAdd)+'px')})}}
var options=$.extend({slice:null,outer:!1,lineheight:!1,autoslicecount:!0,classNull:!1,minHeight:!1,options:!1,resize:!0,},options);var el=$(this);_slice(el)}
...
{propsHtml='<div class="props_item">';$('.buy_block .sku_props .bx_catalog_item_scu > div').each(function(){var title=$(this).find('.bx_item_section_name > span').html();propsHtml+='<div class="prop_item">'+'<span>'+title+(title.indexOf(':')>0?'':': ')+(title.indexOf(':')>0?'':'<span class="val">'+$(this).find('ul li.active > span').text()+'</span>')+'</span>'+'</div>'})
propsHtml+='</div>'}
...
$(this).find('#one_click_buy_form_button').addClass("clicked");var form_url=$(this).attr('action');var bSend=!0;if(window.renderRecaptchaById&&window.asproRecaptcha&&window.asproRecaptcha.key)
{if(window.asproRecaptcha.params.recaptchaSize=='invisible'&&typeof grecaptcha!='undefined')
{if($('#one_click_buy_form').find('.g-recaptcha-response').val())
{bSend=!0}
else{grecaptcha.execute($('#one_click_buy_form').find('.g-recaptcha').data('widgetid'));$(this).find('#one_click_buy_form_button').removeClass("clicked");bSend=!1}}}
if(bSend)
...
if(ocb_files.length)
{var obData=new FormData(),bHasFiles=!1;$.each(ocb_files,function(key,value){if(value)
{bHasFiles=!0;obData.append(key+'_'+value.code,value[0])}});if(bHasFiles)
{$.ajax({url:form_url+'?uploadfiles&orderID='+data.message,type:'POST',data:obData,cache:!1,dataType:'json',processData:!1,contentType:!1,error:function(data,exception){if(data)
...
else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}}
else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}}
...
{var obData=new FormData(),bHasFiles=!1;$.each(ocb_files,function(key,value){if(value)
{bHasFiles=!0;obData.append(key+'_'+value.code,value[0])}});if(bHasFiles)
{$.ajax({url:form_url+'?uploadfiles&orderID='+data.message,type:'POST',data:obData,cache:!1,dataType:'json',processData:!1,contentType:!1,error:function(data){alert('Error with files')},success:function(respond,textStatus,jqXHR){$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}})}
else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}}
else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.QUICK_ORDER)}}
else{$('.one_click_buy_result').show();$('.one_click_buy_result_fail').show();if(('err' in data)&&data.err)
...
script+='?'+paramsStr+'auth=Y';else script+='?'+paramsStr+'data-trigger='+encTriggerAttrs;if(!$('.'+name+'_frame[data-trigger="'+encTriggerAttrs+'"]').length){if(_this.attr('disabled')!='disabled'){$('body').find('.'+name+'_frame[data-trigger="'+encTriggerAttrs+'"]').remove();$('body').append('<div class="'+name+'_frame '+sFastViewClass+' jqmWindow popup" data-trigger="'+encTriggerAttrs+'"></div>');$('.'+name+'_frame[data-trigger="'+encTriggerAttrs+'"]').jqm({trigger:trigger,onLoad:function(hash){onLoadjqm(name,hash)},onHide:function(hash){onHidejqm(name,hash)},ajax:script})}}}}
...
else{var script=arNextOptions.SITE_DIR+'form/';script+='?name='+name+'&form_id=ocb&path='+window.location.pathname+'&ELEMENT_ID='+elementID+'&IBLOCK_ID='+iblockID+'&ELEMENT_QUANTITY='+elementQuantity+'&OFFER_PROPS='+fill_prop.props
...
{$('.fast_order').addClass('clicked');$('body').find('.'+name+'_frame').remove();$('body').find('.'+name+'_trigger').remove();$('body').append('<div class="'+name+'_frame popup"></div>');$('body').append('<div class="'+name+'_trigger"></div>');$('.'+name+'_frame').jqm({trigger:'.'+name+'_trigger',onHide:function(hash){onHidejqm(name,hash)},onLoad:function(hash){onLoadjqm(name,hash)},ajax:arNextOptions.SITE_DIR+'ajax/one_click_buy_basket.php'});$('.'+name+'_trigger').click()}}
else{var script=arNextOptions.SITE_DIR+'form/';script+='?name='+name+'&form_id=ocb&path='+window.location.pathname+'&buy_basket=y';location.href=script}}}
...
{var more=$menuTopest.find('>.more');var moreDropdown=more.find('>.dropdown');var moreWidth=more.actual('outerWidth',{includeMargin:!0});var arItemsHideWidth=[];ignoreResize.push(!0);_init();_gummi();ignoreResize.pop();BX.addCustomEvent('onWindowResize',function(eventdata){try{ignoreResize.push(!0);_gummi()}
catch(e){}
finally{ignoreResize.pop()}})}}}}
...
{var $wrapMenu=$topMenu.parents('.wrap_menu');var $wrapMenuLeft=$wrapMenu.find('.catalog_menu_ext');var more=$topMenu.find('>.more');var moreWidth=more.actual('outerWidth',{includeMargin:!0});more.addClass('hidden');var arItemsHideWidth=[];var moreDropdown=more.find('>.dropdown');var itemsPadding=parseInt(more.find('>a').css('padding-left'))*2;var itemsPadding_min=itemsPadding;ignoreResize.push(!0);_init();_gummi();ignoreResize.pop();BX.addCustomEvent('onWindowResize',function(eventdata){try{ignoreResize.push(!0);_gummi()}
catch(e){}
finally{ignoreResize.pop()}})}}}
...
else if((arNextOptions.COUNTERS.YANDEX_ECOMERCE=="Y"&&arNextOptions.COUNTERS.YANDEX_COUNTER>0)||(arNextOptions.COUNTERS.GOOGLE_ECOMERCE=="Y"&&arNextOptions.COUNTERS.GOOGLE_COUNTER>0)){return!0}
else{return!1}}}
...
i=0,propCollection=null,foundValues=!1,basketParams={},obBasketProps=null;obBasketProps=BX(basket_prop_div);if(!!obBasketProps)
{propCollection=obBasketProps.getElementsByTagName('select');if(!!propCollection&&!!propCollection.length)
{for(i=0;i<propCollection.length;i++)
{if(!propCollection[i].disabled)
{switch(propCollection[i].type.toLowerCase())
{case 'select-one':basketParams[propCollection[i].name]=propCollection[i].value;foundValues=!0;break;default:break}}}}
propCollection=obBasketProps.getElementsByTagName('input');if(!!propCollection&&!!propCollection.length)
{for(i=0;i<propCollection.length;i++)
{if(!propCollection[i].disabled)
{switch(propCollection[i].type.toLowerCase())
{case 'hidden':basketParams[propCollection[i].name]=propCollection[i].value;foundValues=!0;break;case 'radio':if(propCollection[i].checked)
...
callback()}
else{setTimeout(function(){waitCounter(idCounter,delay,callback)},delay)}}
var isOnceInited=insertFilter=!1;var animationTime=200;var delayTime=200;var topMenuEnterTimer=!1;var isMobile=jQuery.browser.mobile;if(isMobile)
...
block=el;if(block.length){var slide=block.closest('.slides');var zoomer=block,options,defaults={position:'#zoomer_in',Xoffset:100,zoomWidth:545,zoomHeight:545,adaptive:!1,title:!1,scroll:!1,};var config=$.extend({},defaults,options,zoomer.data('plugin-options'));zoomer.xzoom(config)}}
var arBasketAsproCounters=arStatusBasketAspro=arBasketPrices={};SetActualBasketFlyCounters=function(){if(arBasketAsproCounters.DEFAULT==!0){$.ajax({url:arNextOptions.SITE_DIR+'ajax/basket_fly.php',type:'post',success:function(html){$('#basket_line .basket_fly').removeClass('loaded').html(html)}})}
...
{var isHeaderFixed=!1,isTabsFixed=!1,headerCanFix=!0,headerFixedHeight=header_fixed.actual('outerHeight'),headerNormalHeight=header.actual('outerHeight'),headerDiffHeight=headerNormalHeight-headerFixedHeight,mobileBtnMenu=$('.btn.btn-responsive-nav'),headerTop=$('#panel:visible').actual('outerHeight');topBlock=$('.TOP_HEADER').first();if(headerDiffHeight<=0)
...
{isHeaderFixed=!0;header_fixed.addClass('fixed');if($('nav.mega-menu.sliced').length)
...
var mfixed=$('.wrapper1.mfixed_Y #mobileheader');if(mfixed.length&&window.matchMedia('(max-width: 991px)').matches)
{var isMHeaderFixed=!1,mheaderCanFix=!0,mheaderFixedHeight=0,mheaderTop=$('#panel:visible').actual('outerHeight');$(window).scroll(function(){var scrollTop=$(window).scrollTop();if($('.wrapper1.mfixed_Y.mfixed_view_scroll_top #mobileheader').length)
...
else{block.parent().removeAttr('style');block.parent().parent().removeAttr('style');if(documentScrollTopLast+diff_top_scroll>offset.top)
...
if(!funcDefined('setPriceItem')){var setPriceItem=function setPriceItem(main_block,quantity,rewrite_price,check_quantity,is_sku){var old_quantity=main_block.find('.to-cart').attr('data-ratio'),value=(typeof rewrite_price!=='undefined'&&rewrite_price?rewrite_price:main_block.find('.to-cart').attr('data-value')),currency=main_block.find('.to-cart').attr('data-currency'),total_block='<div class="total_summ" style="display:none;"><div>'+BX.message('TOTAL_SUMM_ITEM')+'<span></span></div></div>',price_block=main_block.find('.cost.prices'),check=(typeof check_quantity!=='undefined'&&check_quantity);if(main_block.find('.buy_block').length)
...
else{main_block.find('.total_summ span').text(BX.Currency.currencyFormat((value*quantity),currency,!0));if(main_block.find('.total_summ').is(':hidden'))
main_block.find('.total_summ').slideDown(100)}}
else{main_block.find('.total_summ').slideUp(100)}}}}
...
$.cookie('_ym_debug',1,{path:'/',});else $.cookie('_ym_debug',null,{path:'/',});var $mobileMenu=$("#mobilemenu")
...
else $('select.city option').prop('disabled','disabled');$('select.city option:eq(0)').prop('disabled','');$('select.city').ikSelect('reset')}})
...
else{height_block=$(this).closest('.maxwidth-theme').actual('outerHeight');if($(this).closest('.top-block').length)
...
{if($('.inline-search-block').hasClass('show'))
$('<div class="jqmOverlay search"></div>').appendTo('body');else $('.jqmOverlay').detach()}}
...
else{$(document).on('click','*[data-event="jqm"]',function(e){e.preventDefault();e.stopPropagation();if(!$(this).hasClass('clicked'))
{$(this).addClass('clicked');$(this).jqmEx();$(this).trigger('click')}
return!1})}
...
else{$('.basket-link.basket').removeClass('basket-count');$('.basket-link.basket .count').addClass('empted');if($('.basket-link.basket .prices').length)
...
else{$('.basket-link.delay').removeClass('basket-count');$('.basket-link.delay .count').addClass('empted')}
...
else{$('.basket-link.compare').removeClass('basket-count');$('.basket-link.compare .count').addClass('empted');if($('#compare_fly').length)
...
else{slide_block.slideUp(200,function(){container.removeClass('active')})}}
else{container.addClass('active');if(bReviewTab)
{$('#reviews_content').slideDown()}
else{if($('.tabs_section + #reviews_content').length)
...
InitFlexSlider();setTimeout(function(){InitTopestMenuGummi();isOnceInited=!0},50);InitZoomPict();$(document).on('click','.captcha_reload',function(e){var captcha=$(this).parents('.captcha-row');e.preventDefault();$.ajax({url:arNextOptions.SITE_DIR+'ajax/captcha.php'}).done(function(text){captcha.find('input[name=captcha_sid]').val(text);captcha.find('img').attr('src','/bitrix/tools/captcha.php?captcha_sid='+text);captcha.find('input[name=captcha_word]').val('').removeClass('error');captcha.find('.captcha_input').removeClass('error').find('.error').remove()})});if(arNextOptions.PAGES.BASKET_PAGE)
{if(arNextOptions.THEME.SHOW_BASKET_PRINT=="Y")
{if($('.page-top h1').length)
$('<div class="print-link" title="'+arNextOptions.THEME.EXPRESSION_FOR_PRINT_PAGE+'"><i class="svg svg-print"></i></div>').insertBefore($('.page-top h1'))}}
else{if(arNextOptions.THEME.PRINT_BUTTON=='Y')
...
{ratio=Math.round(ratio*arNextOptions.JS_ITEM_CLICK.precisionFactor)/arNextOptions.JS_ITEM_CLICK.precisionFactor;if(Math.round(diff*arNextOptions.JS_ITEM_CLICK.precisionFactor)/arNextOptions.JS_ITEM_CLICK.precisionFactor==ratio)
diff=0}
if($(this).hasClass('focus'))
{intCount=Math.round(Math.round(val*arNextOptions.JS_ITEM_CLICK.precisionFactor/ratio)/arNextOptions.JS_ITEM_CLICK.precisionFactor)||1;val=(intCount<=1?ratio:intCount*ratio);val=Math.round(val*arNextOptions.JS_ITEM_CLICK.precisionFactor)/arNextOptions.JS_ITEM_CLICK.precisionFactor}
...
else{location.href=arNextOptions.SITE_DIR+'auth/?backurl='+location.pathname}}
else{var item=$(this).attr('data-item'),iblockid=$(this).attr('data-iblockid');$('.to-subscribe[data-item='+item+']').hide();$('.to-subscribe[data-item='+item+']').parent().find('.in-subscribe').show();$.get(arNextOptions.SITE_DIR+'ajax/item.php?item='+item+'&subscribe_item=Y',$.proxy(function(data){arStatusBasketAspro={};getActualBasket(iblockid)}));th.removeClass('clicked')}}})
...
else{if(th.closest('.item-views').find('.items').length)
{th.closest('.item-views').find('.items').append(html)}
else{$(html).insertBefore($('.blog .bottom_nav'))}
$('.bottom_nav').html($('.bottom_nav:hidden').html());$('.bottom_nav:hidden').remove()}
var eventdata={action:'ajaxContentLoaded',content:html};BX.onCustomEvent('onCompleteAction',[eventdata,th[0]]);setTimeout(function(){$('.banners-small .item.normal-block').sliceHeight({resize:!1});if($('.item.slice-item').length)
{$('.item.slice-item .title').sliceHeight({resize:!1});$('.item.slice-item').sliceHeight({resize:!1})}
...
else{summ=$('#allSum_FORMATED').html().replace(/&nbsp;/g,' ');title=BX.message('JS_BASKET_COUNT_TITLE').replace('SUMM',summ)}
...
else{className.text(BX.message('FILTER_HIDE_VALUES'));all_props_block.show()}}}}
...
else{_th.removeClass('hover');var href=($(e.target).attr('href')?$(e.target).attr('href'):$(e.target).closest('a').attr('href'));if(href&&href!=='undefined')
location.href=href}})})}}else{$(selector).off()}}
...
{parent='body ';if(typeof popup==='string'&&popup=='Y')
...
window[obProduct].setPriceAction('Y');else setPriceItem(_th.closest('.main_item_wrapper'),_th.val())}
else setPriceItem(_th.closest('.main_item_wrapper'),_th.val())})}}
function initFull(){initSelects(document);initHoverBlock(document);touchItemBlock('.catalog_item a');InitOrderCustom();showTotalSummItem();basketActions();orderActions()}
...
{arProperty=this.validation.properties[id];data=this.getValidationData(arProperty,propContainer);dataPhone=this.getValidationDataPhone(arProperty,propContainer);data=$.extend({},data,dataPhone);propsErrors=propsErrors.concat(this.isValidProperty(data,!0))}}
...
BX.Sale.OrderAjaxComponent.doSaveAction();else BX.Sale.OrderAjaxComponent.oldClickOrderSaveAction(event)}}
...
else{var cur_index=$('.bx_sort_container a.current').index();$('.bx_sort_container .top_control .delete_all').data("type",cur_index);if($('.bx_ordercart > div:eq('+cur_index+') table tbody tr td.item').length)
...
else{if(!$('.icon_error_wrapper').length&&typeof jsPriceFormat!=='undefined'){$('.bx_ordercart_order_pay_center').prepend('<div class="icon_error_wrapper"><div class="icon_error_block">'+BX.message("MIN_ORDER_PRICE_TEXT").replace("#PRICE#",jsPriceFormat(arNextOptions.PRICES.MIN_PRICE))+'</div></div>')}
...
{if(!$('.oneclickbuy.fast_order').length&&arNextOptions.THEME.SHOW_ONECLICKBUY_ON_BASKET_PAGE=="Y"&&!$('.basket-btn-checkout.disabled').length)
$('.basket-checkout-section-inner').append('<div class="fastorder"><span class="oneclickbuy btn btn-default btn-lg fast_order" onclick="oneClickBuyBasket()">'+BX.message("BASKET_QUICK_ORDER_BUTTON")+'</span></div>')}
else{$('.bx_ordercart .bx_ordercart_order_pay .checkout').css('opacity','1');if(!$('.oneclickbuy.fast_order').length&&arNextOptions.THEME.SHOW_ONECLICKBUY_ON_BASKET_PAGE=="Y")
...
var isFrameDataReceived=!1;if(typeof window.frameCacheVars!=="undefined"){BX.addCustomEvent("onFrameDataReceived",function(json){initFull();CheckTopMenuPadding();CheckTopMenuOncePadding();CheckTopMenuDotted();CheckSearchWidth();if($('.logo-row.v2').length)
...
else{$(document).find('.specials.tab_slider_wrapp .tabs_content > li.cur').each(function(){var _th=$(this);_th.css('height','');var itemsButtonsHeight=0;if(_th.find('.tabs_slider li .footer_button').length)
...
timerScroll=setTimeout(function(){BX.onCustomEvent('onWindowScroll',!1)},50)}});BX.addCustomEvent('onWindowResize',function(eventdata){try{ignoreResize.push(!0);CheckTopMenuPadding();CheckTopMenuOncePadding();CheckSearchWidth();CheckTabActive();CheckTopMenuFullCatalogSubmenu();CheckHeaderFixedMenu();CheckTopMenuDotted();if($('nav.mega-menu.sliced').length)
...
finally{ignoreResize.pop()}});BX.addCustomEvent('onWindowScroll',function(eventdata){try{ignoreScroll.push(!0)}
catch(e){}
finally{ignoreScroll.pop()}});BX.addCustomEvent('onSlideInit',function(eventdata){try{ignoreResize.push(!0);if(eventdata){var slider=eventdata.slider;if(slider){if(slider.hasClass('small-gallery'))
...
{eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending');return!0}
if(window.asproRecaptcha.params.recaptchaSize=='invisible'&&typeof grecaptcha!='undefined')
{if($(eventdata.form).find('.g-recaptcha-response').val())
{eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending')}
else{grecaptcha.execute($(eventdata.form).find('.g-recaptcha').data('widgetid'));return!1}}
else{eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending')}
return!0}catch(e){console.error(e);return!0}})