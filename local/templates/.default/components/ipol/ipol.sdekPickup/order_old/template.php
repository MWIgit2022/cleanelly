...
<script>
    var IPOLSDEK_pvz = {
        ...
        buttonPVZ : '<a href="javascript:void(0);" class="SDEK_selectPVZ" onclick="IPOLSDEK_pvz.selectPVZ(\'#id#\',\'PVZ\'); return false;"><?=$linkNamePVZ?></a>',// html ������ "������� ���". Profiler
        ...
        getPrices: function(){
            ...
            else if (curDelivery == 37 || curDelivery == 2)
            {
        ...
        choozePVZ: function(pvzId,isAjax){// ������� ���
            ...
            IPOLSDEK_pvz.pvzAdress=IPOLSDEK_pvz.city+", "+IPOLSDEK_pvz[IPOLSDEK_pvz.curMode][IPOLSDEK_pvz.city][pvzId]['Address'];

            IPOLSDEK_pvz.pvzId = pvzId;

            $('input[name=ORDER_PROP_27]').val(pvzId);
...
<script>
    $( document ).ready(function() {
        $('body').on('click', '.SDEK_selectPVZ', function() {
            if($('.SDEK_all-items').length > 0) {
                if($('.SDEK_all-items').css('display') == 'none') {
                    $('.SDEK_arrow').toggleClass('up');
                    $('.SDEK_all-items').slideToggle(300);
                }
            }
