...
<script>
    var IPOLSDEK_pvz = {
        ...
        getPrices: function(){
			...
			if ($('input[name=ORDER_PROP_28]').val() == 0 || $('input[name=ORDER_PROP_28]').val() == "")
            {
                $('input[name=ORDER_PROP_22]').val("");
            ...
            if (curDelivery == 52)
            {
                ...
                $('input[name=ORDER_PROP_24], input[name=ORDER_PROP_25], input[name=ORDER_PROP_26]').each( function( index, el) {
                    if ($(el).parents(".bx-soa-customer-field").is(":hidden"))
                        $(el).parents(".bx-soa-customer-field").show();
                });

                $('input[name=ORDER_PROP_29]').each( function( index, el) {
            ...
            else if (curDelivery == 53 || curDelivery == 2)
            {
                ...
                if (curDelivery == 2){
                    if ($("input[name=ORDER_PROP_29]").parents(".bx-soa-customer-field").is(":hidden"))
                        $("input[name=ORDER_PROP_29]").parents(".bx-soa-customer-field").show();
                    if ($("input[name=ORDER_PROP_22]").parents(".bx-soa-customer-field").is(":visible"))
                        $("input[name=ORDER_PROP_22]").parents(".bx-soa-customer-field").hide();
            ...
            //Проверка, если город в запросе не равен городу в скрытом поле обновляем CDEK ID
            if ($('input[name=ORDER_PROP_28]').val() == 0) {
                $('input[name=ORDER_PROP_5]').val(IPOLSDEK_pvz.city);
                $.ajax({
                    method: 'POST',
                    dataType: 'html',
                    url: "/local/ajax/getCDEKCityID.php",
                    data: {cityID:IPOLSDEK_pvz.cityID},
            ...
            // if (IPOLSDEK_pvz.city != $('input[name=ORDER_PROP_5]').val())
            //     $('input[name=ORDER_PROP_5]').val(IPOLSDEK_pvz.city);
        ...
        cityPVZHTML: function(){
            ...
            for(var i in IPOLSDEK_pvz.cityPVZ){
                ...
                html+='<p id="PVZ_'+i+'" data-addr="'+IPOLSDEK_pvz.city+', '+IPOLSDEK_pvz.cityPVZ[i].Address+'" onclick="IPOLSDEK_pvz.markChosenPVZ(\''+i+'\')" onmouseover="IPOLSDEK_pvz.Y_blinkPVZ(\''+i+'\',true)" onmouseout="IPOLSDEK_pvz.Y_blinkPVZ(\''+i+'\')">'+IPOLSDEK_pvz.paintPVZ(i)+'<span class="for_mob_span">'+IPOLSDEK_pvz.city+', '+IPOLSDEK_pvz.cityPVZ[i].Address+'</span></p>';
            }
		...
		Y_filterPVZ: function(wat,ifOn){
			if(typeof(ifOn)!=='undefined' && ifOn){
				console.log(wat);
				IPOLSDEK_pvz.cityPVZ[wat].placeMark.options.set({iconImageSize: [40, 43]});
			} else {
				IPOLSDEK_pvz.cityPVZ[wat].placeMark.options.set({iconImageSize: [0, 0]});
            }
        },
        Y_clearPVZ: function(){
	...
	$(document).on('click', '.sdek_custim_tabs span', function(){
		$('.sdek_custim_tabs span').removeClass('active');
		$(this).addClass('active');
	})
	$(document).on('input', '.sdek_custim_tabs input', function(){
		var inp = $(this).val().toLowerCase();
		$(this).closest('#SDEK_info').find('.SDEK_all-items p').each(function(){
			var pvz = $(this).attr('id').replace('PVZ_','');
			if($(this).data('addr').toLowerCase().indexOf(inp) != -1){
				$(this).show();
				IPOLSDEK_pvz.Y_filterPVZ(pvz,true);
			} else {
				$(this).hide();
				IPOLSDEK_pvz.Y_filterPVZ(pvz,false);
			}
			$('#SDEK_info .SDEK_all-items').css('opacity','1');
		})
	})
	$(document).on('focus', '.sdek_custim_tabs input', function(){
		$('#SDEK_info .SDEK_all-items').show().css('opacity','1');
	})
	$(document).on('focusout', '.sdek_custim_tabs input', function(){
		if(window.innerWidth<919){
			setTimeout(function(){
				$('#SDEK_info .SDEK_all-items').hide();
			},500);
		}
	})
	function customIpolPostamat(){
		IPOLSDEK_pvz.close();
		window.SDEK_CUSTOM = ['POSTAMAT', 54];
		$('#ID_DELIVERY_ID_54').click();
		return false;
	}
	function customIpolPvz(){
		IPOLSDEK_pvz.close();
		window.SDEK_CUSTOM = ['PVZ', 53];
...
<div id='SDEK_pvz'>
    ...
    <div id='SDEK_info'>
		...
		<div class="sdek_custim_tabs">
			<span class="pvz active" onclick="customIpolPvz()"><?=getMessage('PVZ');?></span>
			<span class="postamat" onclick="customIpolPostamat()"><?=getMessage('POSTAMAT');?></span>
			<input placeholder="<?=getMessage('SEARCH_PLACEHOLDER');?>">
		</div>
