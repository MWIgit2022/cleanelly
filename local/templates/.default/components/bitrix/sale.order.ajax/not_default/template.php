...
<input type='hidden' id='freeDeliveryThreshold' name='freeDeliveryThreshold' value='<?= HBUtils::GetSettings("settings")['FREE_DELIVERY_THRESHOLD']['VALUE'] ?>'>
<?use Bitrix\Main,
...
if (strlen($request->get('ORDER_ID')) > 0)
{
	...
	<script type="text/javascript">
		...
		_rutarget.push({'event': 'thankYou', 'conv_id': 'order', 'order_id': <?=$request->get('ORDER_ID')?>, 'total_cost': <?=$arResult["ORDER"]["PRICE"]?>});
	</script>
	<?
}
...
else
{
	...
	<?foreach($arResult['JS_DATA']['GRID']['ROWS'] as $item){
		if($item['data']['CUSTOM_PRICE'] == 'Y'){
			$arResult['JS_DATA']['TOTAL']['CUSTOM_PRICE_CHANGED'] = 'Y';
		} else {
			$arResult['JS_DATA']['TOTAL']['CUSTOM_PRICE_CHANGED'] = 'N';
		}
	}?>
...
<script type="text/javascript" >
	...
	var refreshOrderEvent = new Event('refreshOrder');
...
<?//if($USER->isAdmin()){?>
	<div class="modal" id="online_payment_discount" style="display:none;"> 
		<div class="payment_section_discount">
			<p class="header_modal">Скидка при оплате на сайте: <span class="percent_discount">5%</span> применена!</p>
