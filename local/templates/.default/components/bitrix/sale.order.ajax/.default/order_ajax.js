...
console.log(result.order); */
				...
				setTimeout(function(){
					if(!!result.sdek){
						...
						$('input[name=ORDER_PROP_5]').val(0);
						var sdek_cityId = 0;
					}
					$.ajax({
						method: 'POST',
						dataType: 'html',
						url: "/local/ajax/getCDEKCityID.php",
						data: {cityID:	sdek_cityId},
						success: function(msg) {
							if (msg)
								$('input[name=ORDER_PROP_28]').val(msg);
							else 
								$('input[name=ORDER_PROP_28]').val(0);
						} 
					});
				}, 2000);
				// this.checkCitiesOnPickUp(result, actionAfterAjax);
				
				// var self = this;

				// function actionAfterAjax(result) {
				// 	self.isPriceChanged(result);
				// 	if (self.activeSectionId !== self.deliveryBlockNode.id)
				// 	self.deliveryCachedInfo = [];

				// 	self.result = result.order;
				// 	self.prepareLocations(result.locations);
				// 	self.locationsInitialized = false;
				// 	self.maxWaitTimeExpired = false;
				// 	self.pickUpMapFocused = false;
				// 	self.deliveryLocationInfo = {};
				// 	self.initialized = {};

				// 	self.initOptions();
				// 	self.editOrder();
				// 	self.mapsReady && self.initMaps();
				// 	BX.saleOrderAjax && BX.saleOrderAjax.initDeferredControl();
				// }
			}

			//this.fieldsForCardProcessing();

			if(!refreshOrderEvent) {
				refreshOrderEvent = new Event('refreshOrder');
			}
			document.body.dispatchEvent(refreshOrderEvent);

			$('#soa-property-31').inputmask("99.99.9999", {
				"placeholder": ' '
			});

			$('#soa-property-31').on('input', function() {
				var dateParts = $(this).val().split('.');
				var momentJsFormat = 'DD.MM.YYYY';
				//if date is full
				if($(this).val().length > 0 && !$(this).val().includes(' ')) {
					var isValid = moment($(this).val(), momentJsFormat).isValid();
					//if date is incorrect OR more than today than replace value as today
					var setDefault = !isValid || moment($(this).val(), momentJsFormat) > moment();	
					if(setDefault) {
						$(this).val(moment(new Date()).format(momentJsFormat));
					}
				} else {
					//exclude day more than 31 and month > 12
					if (dateParts[0] > 31) {
						$(this).val('31')
					}

					if (dateParts[1] > 12) {
						$(this).val($(this).val().slice(0,3)+'12'+$(this).val().slice(5))
					}
				}
			});
		...
		checkCitiesOnPickUp: function(result, actionAfterAjax)
		{	
			var city = result.sdek.city;
			var delivery = result.order.DELIVERY;
			
			BX.ajax({
				method: 'POST',
				dataType: 'json',
				url: "/local/ajax/checkCitiesOnPickUp.php",
				data: {city:city},
				onsuccess: BX.delegate(function(msg) {
					if (!msg)
					{
						if (delivery[2].CHECKED == "Y")
							delivery[36].CHECKED = "Y";
						delete delivery[2];
						BX.Sale.OrderAjaxComponent.deactivatePickUp();
					}else{
						delivery[2].ADRESSES = msg;
					}
					result.order.delivery = delivery;
					
					actionAfterAjax(result);
				}, this)
			});
		},
		...
		initPhoneField: function() 
		{
			...
			$('input[name=ORDER_PROP_3').change(function() {
		...
		isPriceChanged: function(result)
		{
			...
			//кастомное применение скидки при оплате картой, если есть нестандартная акция, через кастомную цену.
			 if(parseFloat(priceBefore) == parseFloat(priceAfter)){
				...
				if(custom_disc){
					/* priceAfter = priceBefore - priceBefore/100*5;
			...
			if(this.result.PAY_SYSTEM[0].CHECKED !='Y' && result.order.PAY_SYSTEM[0].CHECKED == 'Y'){
				setTimeout(function(){
					$th.showPopup();
				},200);
			}
		...
		createBasketItemContent: function(data)
		{
			...
			if (this.options.showPropsInBasket && props.length)
			{
				for (var i in props)
				{
					if (props.hasOwnProperty(i) && props[i].CODE != 'promo50e2' && props[i].CODE != 'cheap')
					{
		...
		createPaySystemItem: function(item)
		{
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y')
			{
				title = BX.create('DIV', {props: {className: 'bx-soa-pp-company-smalltitle'}, text: item.NAME});
                        ...
                        subtext = BX.create('DIV', {
                                props: {className: 'bx-soa-pp-company-smalltitle_descr'},
                                html: item.DESCRIPTION 
                        });

			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-4 col-sm-4 col-xs-6'},
				children: [label, title, subtext],
				events: {
...
//				});
		...
		showPopup: function()
		{
			if($('#online_payment_discount').length>0 && this.result.TOTAL.ORDER_PRICE>=5000){
				$('#all_discount_price').text(this.result.TOTAL.BASKET_PRICE_DISCOUNT_DIFF);
				$.fancybox.open($('#online_payment_discount').html());	
			}
		},
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			infoList = BX.create('UL', {props: {className: 'bx-soa-pp-list'}, children: [price]});
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, subtext, price, itemNode, logoNode;
			...
			if (item.ADRESSES)
			{
				...
				var adress = BX.create('option', {
					props: {className: 'bx-soa-pp-company-smalltitle_descr first-option', value: "novalue"},
					html: "Выберите пункт самовывоза"
				});

				select_body.push(adress);

				for (var key in item.ADRESSES) {
					
					var adress = BX.create('option', {
						props: {className: 'bx-soa-pp-company-smalltitle_descr', value: item.ADRESSES[key]},
						html: item.ADRESSES[key]
					});
				...
				var select = BX.create('select', {
					props: {className: 'bx-soa-pp-company-select'},
					children: select_body,
				});
				arr.push(select);
			}
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			var propsItemNode = BX.create('DIV'),
				...
				label,
				propsCode = ["CODE_PVZ", "CITY_CDEK","CHECKED_DELIVERY"];
			...
			{
				if (propsCode.includes(property.getSettings().CODE))
					BX.addClass(propsItemNode, "form-group bx-soa-customer-field hidden");
				else
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.CODE == 'ADRESS_PICKUP' || settings.CODE == 'ADDRESS_PVZ')
		...
		alterDateProperty: function(settings, inputText)
		{
			...
			addon = BX.create('DIV', {
				props: {className: 'input-group-addon date-property'},
		...
		getValidationData: function(arProperty, propContainer)
		{
			...
			var data = {}, inputs;
			let customerFieldCheckbox = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_30][type!=hidden]');
			let customerFieldDate = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_31][type!=hidden]');
			if(customerFieldCheckbox && customerFieldDate) {
				if(customerFieldCheckbox.prop("checked")) {
					var fieldCheckingResult = this.checkBirthdateField(customerFieldDate);
					if (fieldCheckingResult) {
						if (arProperty.CODE === "BIRTH") {
						arProperty.REQUIRED = "Y";
						}
					}
				} else {
					if (arProperty.CODE === "BIRTH") {
						arProperty.REQUIRED = "N";
					}
				}
			}
		...
		validateDate: function(input, arProperty, fieldName)
		{
			...
			if (arProperty.REQUIRED == 'Y') {
				if (value.length == 0) {
					errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
				} else if (value.split('.')[2] > new Date().getFullYear() || value.split('.')[2] < new Date().getFullYear() - 120) {
					errors.push(BX.message('JS_DATE'));
				}
			}
				...
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
                        ...
                        if (total.ORDER_PRICE < $('#freeDeliveryThreshold').val())
                        {
                            this.totalInfoBlockNode.appendChild(
                                    BX.create('DIV', {
                                            props: {className: 'bx-soa-cart-total-free_delivery'},
                                            children: [
                                                    BX.create('div', {
                                                            props: {
                                                                    className: 'bx-soa-cart-total-free_delivery_title'
                                                            },
                                                            html: "До бесплатной доставки осталось:"
                                                    }),
                                                    BX.create('div', {
                                                            props: {
                                                                    className: 'bx-soa-cart-total-free_delivery_text'
                                                            },
                                                            html: Math.ceil($('#freeDeliveryThreshold').val()-total.ORDER_PRICE)+" руб."
                                                    })
		...
		checkCouponExistense: function()
		{
			var $coupons = this.result.COUPON_LIST;
			var isCoupon = false;
			...
			if ($coupons.length > 0 ) {
				$.each($coupons, function() {
					...
					if (currentCopon.JS_STATUS == "APPLIED") {
						if (/[0-9]{13}/.test(currentCopon.COUPON)) {
							isCoupon = true;
						}
					}
				});
		...
		couponFieldUpdate: function()
		{
			let customerFieldNumber = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_33][type!=hidden]');		
			$('body').on('click', '.order-coupon-block-coupon-btn', function(){
				let valueOfField = customerFieldNumber.val();				
				if (/[0-9]{13}/.test(valueOfField)) {
					let urlAjax = "/ajax/applyCouponOnOrderPage.php";
					BX.ajax.loadJSON(urlAjax,{
						AJAX:'Y',
						data: valueOfField,
						action: 'addCoupon'
					}, function(data){
		...
		deleteCouponFromForm: function()
		{
			$('body').on('click', '.close-link', function(){
				let urlAjax = "/ajax/deleteCouponOnOrderPage.php";
				let valueOfField = $(this).data('coupon');
				BX.ajax.loadJSON(urlAjax,{
					AJAX:'Y',
					data: valueOfField,
					action: 'deleteCoupon'
		...
		deleteCoupons: function() 
		{
			...
			$.each($coupons, function() {
				var currentCopon = $(this)[0];
				var couponValue = currentCopon.COUPON;
				
				if (/[0-9]{13}/.test(couponValue)) {
					let urlAjax = "/ajax/deleteCouponOnOrderPage.php";
					BX.ajax.loadJSON(urlAjax,{
						AJAX:'Y',
						data: couponValue,
						...
						if (data) {
							$('body').find('.order-coupon-alert-section').hide();
							var $pc = $('.order-promocode-alert')
							$pc.each(function(){
								var attr = $(this).attr('data-promocodediv')
								var attrNum = /[0-9]{13}/.test(attr)
								if(attrNum){
									$(this).hide()
								}
							})
		...
		getSdekID: function(sdek){
			$('input[name=ORDER_PROP_5]').on('focusout',function(){
				BX.Sale.OrderAjaxComponent.sendRequest('refreshOrderAjax');  
			  });
		},

		saveBirthDate: function() {
			var customerFieldDate = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_30][type!=hidden]');
			$('body').on('change', customerFieldDate, function(){
				document.cookie = "birthdate" + "=" +customerFieldDate.val();
			}); 
        },
		...
		checkBirthdateField: function(customerFieldDate) {			
			var field = false;
			var fieldDateName = document.querySelector('label[for="soa-property-30"]').textContent;
			if (customerFieldDate.val() == "") {
				field = true;
			}
		...
		fieldsForCardProcessing: function() {
			var isCoupon = false;
			isCoupon = this.checkCouponExistense(isCoupon);
		
			let customerFieldCheckbox = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_30][type!=hidden]');
			let customerFieldDate = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_31][type!=hidden]');	
			let customerFieldCardCheckbox = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_32][type=checkbox]');
			let customerFieldNumber = $('body').find('.bx-soa-customer-field input[name=ORDER_PROP_33][type!=hidden]');
			var $arr = $('body').find('.bx-soa-customer-field');
			var $coupons = this.result.COUPON_LIST;
		
			this.saveBirthDate();
			var birthDate = this.getCookie("birthdate");
			
			if (birthDate && birthDate != "" && birthDate != "undefined") {
				customerFieldCheckbox.prop('checked', true);
				customerFieldDate.val(birthDate);
			}
			
			var couponValue = false;
			$.each($coupons, function() {
				var currentCopon = $(this)[0];
				if (currentCopon.JS_STATUS == "APPLIED") {
					couponValue = currentCopon.COUPON;
				}
			});
			
			if (isCoupon && /[0-9]{13}/.test(couponValue)) {	
				customerFieldCheckbox.prop('checked', false);
				customerFieldCardCheckbox.prop('checked', true);
				$('body').find('.bx-soa-customer-field input[name=ORDER_PROP_33]').val(couponValue);			
				$arr.each(function () {
					if ($(this).attr('data-property-id-row') == 30) {
						$(this).hide();
					}
					if ($(this).attr('data-property-id-row') == 33) {
						var target = $(this).find('.soa-property-container');
						target.append('<div class="order-coupon-alert-section"></div>');
						target.find('.order-coupon-alert-section').append('<div class="order-coupon-alert-inner"></div>');
						...
						$.each($coupons, function() {
							...
							if (currentCopon.JS_STATUS == "APPLIED") {
								...
								couponAlertInner.find('.order-text-muted.order-text-muted').append('<span class="close-link" data-entity="order-coupon-delete" data-coupon="'+currentCopon.COUPON+'">Удалить</span>');
							} else if (currentCopon.ACTIVE == "N") {
			...
			} else if (!/[0-9]{13}/.test(couponValue)) {
				$arr.each(function () {
					if ($(this).attr('data-property-id-row') == 33) {
						var target = $(this).find('.soa-property-container');
						target.append('<div class="order-coupon-alert-section"></div>');
				...
				$('body').find('.bx-soa-customer-field input[name=ORDER_PROP_33]').val("");
			}
			
			var BXFormPostingObject = this;
			
			if(customerFieldCheckbox && customerFieldDate) {
				let propDateField = customerFieldDate.parents('.bx-soa-customer-field');
				if(customerFieldCheckbox.prop("checked")) {
					propDateField.show();
					customerFieldNumber.val("");
				} else {
					propDateField.hide();
					customerFieldDate.val("");
					this.deleteCookie("birthdate");
				}
				$('body').on('click', customerFieldCheckbox, function() {
					if(customerFieldCheckbox.prop("checked")) {
						propDateField.show();
						...
						propDateField.hide();
						customerFieldDate.val("");
						BXFormPostingObject.deleteCookie("birthdate");
					}
				});
			}
			
			if (customerFieldCardCheckbox && customerFieldNumber) {	
				let propNumberField = customerFieldNumber.parents('.bx-soa-customer-field');
				if(customerFieldCardCheckbox.prop("checked")) {					
					propNumberField.show();
					$arr.each(function () {
						if ($(this).attr('data-property-id-row') == 33) {
							var target = $(this).find('.soa-property-container');
							target.css( 'position', 'relative' );
							target.append('<span class="order-coupon-block-coupon-btn"></span>');
						}
					});	
				} else {
					propNumberField.hide();
				} 
				$(customerFieldCardCheckbox).on('click', function() {
					if(customerFieldCardCheckbox.prop("checked")) {
						propNumberField.show();
						$arr.each(function () {
							if ($(this).attr('data-property-id-row') == 33) {
						...
						$('body').find('.order-coupon-block-coupon-btn').hide();
						propNumberField.hide();	
						$arr.each(function () {
							if ($(this).attr('data-property-id-row') == 30) {
								$(this).show();
						...
						customerFieldNumber.val("");
						BXFormPostingObject.deleteCoupons();
					} 
				});
			}
			
			this.couponFieldUpdate();
			this.deleteCouponFromForm();
		}
	};
})();