...
//				});
			...
			if ($coupons.length > 0 ) {
				$.each($coupons, function() {
					...
					//if (currentCopon.JS_STATUS == "APPLIED") {
						if (/[0-9]{7}/.test(currentCopon.COUPON)) {
							isCoupon = true;
						}
					//}
				});
		...
		fieldsForCardProcessing: function() {
			...
			if (isCoupon && /[0-9]{7}/.test(couponValue)) {	
