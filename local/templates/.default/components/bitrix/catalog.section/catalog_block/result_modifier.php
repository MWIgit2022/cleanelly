...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$productPictures = CIBlockPriceTools::getDoublePicturesForItem($arItem, 'N');

		if (empty($productPictures['PICT'])){
			foreach ($arItem['OFFERS'] as $arOffer)
			{
				if($arOffer['PREVIEW_PICTURE']){
					$productPictures['PICT'] = $arOffer['PREVIEW_PICTURE'];
				}
				if($arOffer['DETAIL_PICTURE']){
					$productPictures['PICT'] = $arOffer['DETAIL_PICTURE'];
				}
			}
		}
