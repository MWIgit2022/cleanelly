	...
	BX.Sale.BasketComponent = {
		...
		applyBasketResult: function(result)
		{
			...
			if (result.BASKET_ITEM_RENDER_DATA)
			{
				...
				/* for (i in result.GRID.ROWS)
				{
					if(result.GRID.ROWS)[i].FULL_PRICE>result.GRID.ROWS)[i].PRICE){
							result.GRID.ROWS)[i].DISCOUNT_PRICE = result.BASKET_ITEM_RENDER_DATA[i].FULL_PRICE - result.BASKET_ITEM_RENDER_DATA[i].PRICE;
							result.BASKET_ITEM_RENDER_DATASHOW_DISCOUNT_PRICE[i].DISCOUNT_PRICE_FORMATED = result.BASKET_ITEM_RENDER_DATA[i].FULL_PRICE - result.BASKET_ITEM_RENDER_DATA[i].PRICE;
						}
				} */
				for (i in result.BASKET_ITEM_RENDER_DATA)
				{
					...
					{
						if(result.BASKET_ITEM_RENDER_DATA[i].FULL_PRICE>result.BASKET_ITEM_RENDER_DATA[i].PRICE){
							result.BASKET_ITEM_RENDER_DATA[i].SHOW_DISCOUNT_PRICE = true;
							result.BASKET_ITEM_RENDER_DATA[i].SUM_DISCOUNT_PRICE = result.BASKET_ITEM_RENDER_DATA[i].SUM_FULL_PRICE - result.BASKET_ITEM_RENDER_DATA[i].SUM_PRICE;
							result.BASKET_ITEM_RENDER_DATA[i].SUM_DISCOUNT_PRICE_FORMATED = numberWithCommas(result.BASKET_ITEM_RENDER_DATA[i].SUM_FULL_PRICE - result.BASKET_ITEM_RENDER_DATA[i].SUM_PRICE)+' руб.';
						}
			...
			if (itemData.CHECK_MAX_QUANTITY === 'Y')
			{
				...
				if (availableQuantity > 0 && quantity > availableQuantity)
				{
					quantity = availableQuantity;
                                        ...
                                        var maxNotify = $('#basket-item-quantity-'+itemData.ID).parents('.basket-item-block-amount').find('.counter_block_ismax');
                                        maxNotify.show();
