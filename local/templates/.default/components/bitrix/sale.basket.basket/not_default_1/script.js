...
BX.ready(function() {
	...
	if (BX.type.isNotEmptyString(basketJSParams['EVENT_ONCHANGE_ON_START']) && basketJSParams['EVENT_ONCHANGE_ON_START'] == "Y")
		BX.onCustomEvent('OnBasketChange');
	console.log(BX.Sale.BasketComponent.result.BASKET_ITEM_RENDER_DATA[0].PROPS[0]);
	
});
BX.addCustomEvent('onAjaxSuccess', function(){
			
			$('.basket-item-property_GIFT_NUMB').each(function(i,v){
				
				if(BX.Sale.BasketComponent.result.allSum<$('#gift_block').data('price')){
					$(this).closest('tr').remove();
				} else if($(this).closest('tr').find('.sale_alert').length == 0){
					$(this).closest('tr').find('.basket-items-list-item-amount').html('<p class="sale_alert">'+$(this).find('.basket-item-property-value').text()+'</p>');
				}
				
			})
			var sale_number = false;
			$('.basket-item-property_SALE_NUMBER').each(function(i,v){
				if($(this).closest('tr').find('.sale_alert').length == 0){
					$(this).closest('tr').find('.basket-item-price-current').last().append('<p class="sale_alert">Акция '+$(this).find('.basket-item-property-value').text()+' применена</p>');
					sale_number = $(this).find('.basket-item-property-value').text();
				}
			})
