...
if (empty($arResult['ERROR_MESSAGE']))
{
	...
	<div id="gift_block" data-price="<?=$settings['GIFT_MIN_SUM']['VALUE']?>">
	...
	$gifts = $settings['GIFTS']['VALUE'];
	$price_on_discount = 0;
	$products_with_gifts = $settings['PRODUCTS_WITH_GIFTS']['VALUE'];
	if($settings['GIFTS_LOGIC']['VALUE'] == 'На выбранные товары'){
		foreach($arResult['GRID']['ROWS'] as $k=>$v){
			...
			if(in_array($prod,$products_with_gifts)){
				$price_on_discount+=$v['PRICE'];
			}
		}
		if($gifts_price>$price_on_discount){
				$mxResult = CCatalogSku::GetProductInfo($gifts[$k]);
				if (is_array($mxResult)) {
					$gift = $mxResult['ID'];
				} else {
					$gift = $gifts[$k];
				...
				<p class="h3"> Чтобы получить подарок, добавьте к заказу товары, которые участвуют в акции, на <?=($settings['GIFT_MIN_SUM']['VALUE']-$price_on_discount)?> руб</p>
				
		<?}
	} elseif($settings['GIFTS_LOGIC']['VALUE'] == 'На все'){
		$price_on_discount = $arResult['TOTAL_RENDER_DATA']['PRICE'];
			if($gifts_price>$price_on_discount){
				$mxResult = CCatalogSku::GetProductInfo($gifts[$k]);
				if (is_array($mxResult)) {
					$gift = $mxResult['ID'];
				...
				<p class="h3"> Чтобы получить подарок добавьте к заказу товаров на <?=($settings['GIFT_MIN_SUM']['VALUE']-$arResult['TOTAL_RENDER_DATA']['PRICE'])?> руб</p>
...
<div class="modal" id="modal_discount" style="display:none;"> 
		<div class="payment_section_discount">
			<p class="header_modal" style="margin-top:1em">Акция <span class="percent_discount action_name"></span> применена!</p>
			<p class="discount_sum">Общая скидка на Ваш заказ: <span id="all_discount_price">100 руб.</span></p>
		</div>
	</div>