...
/**
 * Класс эмулирует ручную генерацию sitemap
 * 
 */
class GenerateSitemap
{
    ...
    /**
     * Метод генерирует sitemap
     * 
     * @param int $id - идентификатор генерации sitemap
     * @param string $action - название файла для генерации
     * 
     */
    public static function generate($id = 1, $action = 'sitemap_run')
    {        
        ...
        if ($action === 'sitemap_run') // && check_bitrix_sessid()
        {
            $arValueSteps = array(
                'init'         => 0,
                'init1'        => 1,
            ...
            foreach ($arValueSteps as $key => $v) {
                ...
                $NS = $request->get('NS') && is_array($request->get('NS')) ? $request->get('NS') : (array)$NS;
            }
